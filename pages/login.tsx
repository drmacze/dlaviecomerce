  ...
  return (
    <main className="dl-login-page min-h-screen overflow-hidden px-3 py-3 text-white md:px-6 md:py-6">
      <style jsx global>{`
        ...
        .auth-accent-card{position:relative;overflow:hidden;border:1px solid rgba(255,255,255,.1)}.auth-accent-card:before{content:'';position:absolute;inset:-2px;border-radius:inherit;padding:2px;background:conic-gradient(from 0deg,rgba(69,213,255,0),rgba(69,213,255,.95),rgba(123,92,255,.95),rgba(223,255,79,.98),rgba(69,213,255,0) 38%);-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask-composite:exclude;animation:authBorderOrbit 3.8s linear infinite;pointer-events:none}.auth-accent-card:after{content:'';position:absolute;inset:1px;border-radius:inherit;border:1px solid rgba(255,255,255,.08);pointer-events:none}
      ...
      <section className="mx-auto grid min-h-[calc(100vh-1.5rem)] max-w-7xl gap-4 lg:grid-cols-[1.08fr_.92fr]">
        ...
        <aside className="dl-login-glass dl-login-ring relative overflow-hidden rounded-[2.35rem] p-4 md:p-6">
          ...
          <div className="relative z-10 flex min-h-full flex-col">
            ...
            <div className="auth-accent-card mt-6 rounded-[1.65rem] bg-[#030712]/86 p-4 text-white shadow-[0_24px_64px_rgba(0,0,0,.24)]"><div className="relative z-10 flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#dfff4f]">Account Login</p><p className="mt-2 text-2xl font-black tracking-tight">Masuk ke akun kamu.</p><p className="mt-1 text-xs font-bold text-white/48">Akses wallet, transaksi, reward, dan dashboard.</p></div><button type="button" onClick={() => setFormOpen((value) => !value)} className="grid h-12 w-12 place-items-center rounded-full bg-[#dfff4f] text-slate-950 shadow-[0_0_28px_rgba(223,255,79,.38)] transition hover:-translate-y-1" aria-label="Toggle form"><svg viewBox="0 0 24 24" className={`h-5 w-5 transition-transform ${formOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9l6 6 6-6" /></svg></button></div><div className="relative z-10 dl-login-progress mt-4 h-1 overflow-hidden rounded-full bg-white/10"><span className="block h-full w-full rounded-full bg-[#dfff4f]" /></div></div>
