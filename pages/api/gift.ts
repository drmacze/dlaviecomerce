...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!receiverEmail || points <= 0) return res.status(400).json({ error: 'Receiver email and amount are required' });
    ...
    if (sender.data.l_points < points) return res.status(400).json({ error: 'L-Points tidak cukup.' });
    const receiver = await supabase.from('profiles').select('*').eq('email', String(receiverEmail).toLowerCase()).single();
