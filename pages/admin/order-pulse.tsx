...
export default function OrderPulse() {
  ...
  async function mark(id: string, next: Order['status']) {
    if (!token) return setStatus('Session admin tidak ditemukan. Silakan login ulang.');
    ...
    setStatus(response.ok ? 'Order diperbarui dan audit log dicatat.' : data.error || 'Gagal memperbarui order.');
    await load();
  }
