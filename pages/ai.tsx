...
type UiAttachment = { id: string; name: string; kind: 'file' | 'photo'; size: number };
...
const pricingCards = [
  { name: 'Basic', price: 'Gratis', badge: 'Starter', accent: 'from-slate-200/20 to-white/5', points: ['Dlavie X Lite / Mini untuk chat ringan', 'Cocok untuk ide, ringkasan, dan bantuan umum', 'Tanpa Memory aktif', 'Tanpa Build agent lanjutan', 'Token usage paling hemat'] },
  { name: 'Core', price: 'AI Token', badge: 'Recommended', accent: 'from-[#dfff4f]/25 to-blue-500/10', points: ['Thinking mode dan Power/Core agent mode', 'Memory dapat diaktifkan', 'Build workspace dan Gems premium', 'Model Dlavie 1.5 / X 3', 'Cocok untuk coding, strategi, dan workflow serius'] },
  { name: 'Studio', price: 'Custom', badge: 'Team', accent: 'from-violet-500/25 to-cyan-400/10', points: ['Multi project dan workspace tim', 'Custom model routing', 'Priority resource generation', 'Agent automation pipeline', 'Dirancang untuk produk digital skala besar'] },
  { name: 'Enterprise', price: 'Contact', badge: 'Scale', accent: 'from-orange-500/25 to-[#dfff4f]/10', points: ['Private workflow governance', 'Custom memory policy', 'Audit-ready usage log', 'Dedicated AI runtime strategy', 'Support integrasi sistem internal'] },
];
...
export default function AI() {
  ...
  const memoryLocked = (access?.plan || 'basic') === 'basic';
        ...
        <section id="dlavie-ai-console" className="min-h-screen bg-[#0b0c10]">
          ...
          <div className="mx-auto grid max-w-7xl gap-4 px-4 py-4 lg:grid-cols-[230px_1fr_330px]">
            ...
            <section className="overflow-hidden rounded-[1.8rem] border border-white/10 bg-white/[0.055] shadow-2xl shadow-black/30">
              {tab === 'chat' && <ChatPanel messages={messages} busy={busy} q={q} setQ={setQ} ask={ask} selectedModel={selectedModel} activePlan={activePlan.name} estimatedPromptUnits={estimatedPromptUnits} planMode={planMode} setPlanMode={setPlanMode} runtimeMode={runtimeMode} setRuntimeMode={setRuntimeMode} modes={modes} attachments={attachments} setAttachments={setAttachments} webSearch={webSearch} setWebSearch={setWebSearch} memoryEnabled={memoryEnabled} memoryLocked={memoryLocked} />}
              ...
              {tab === 'pricing' && <PricingPanel cards={pricingCards} />}
