    ...
    <section className="mt-3 rounded-[1.55rem] bg-white/[.07] p-3 ring-1 ring-white/10">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm font-bold text-white/55">Pilih metode, lalu buat pending.</p>
        </div>
      ...
      <article className="relative mt-2 overflow-hidden rounded-[1.45rem] bg-slate-950/80 p-4 ring-1 ring-white/10">
        ...
        {active.id === 'qris' ? (
          <div className="mt-4 grid grid-cols-[7.5rem_1fr] gap-3">
            <div className="relative rounded-[1.1rem] bg-white p-2 shadow-[0_0_32px_rgba(223,255,79,.16)]">
              <div className="grid grid-cols-12 gap-[2px] rounded-lg bg-white p-1">
                {Array.from({ length: 144 }).map((_, index) => <span key={index} className={`aspect-square rounded-[1px] ${qrCells.includes(index) ? 'bg-slate-950' : 'bg-transparent'}`} />)}
              </div>
              <p className="mt-1 text-center text-[9px] font-black text-slate-950">QRIS</p>
            </div>
            <div className="rounded-[1.1rem] bg-white/10 p-3 ring-1 ring-white/10">
              <p className="text-xs font-black text-[#dfff4f]">TOKOBOT</p>
              <p className="mt-1 text-xs font-semibold leading-5 text-white/55">Scan QRIS, isi nominal sama persis, lalu tekan buat pending.</p>
            </div>
          </div>
        ) : (
          <div className="mt-4 rounded-[1.1rem] bg-white/10 p-3 ring-1 ring-white/10">
            <p className="text-[10px] font-black uppercase tracking-widest text-white/35">Nomor tujuan</p>
            <p className="mt-1 break-all text-lg font-black text-[#dfff4f]">{active.number}</p>
          </div>
        )}

        <button onClick={onCreateTopup} className="mt-4 w-full rounded-full bg-[#dfff4f] px-5 py-3.5 text-sm font-black text-slate-950 shadow-[0_14px_34px_rgba(223,255,79,.18)] transition hover:-translate-y-1">Buat Topup Pending</button>
