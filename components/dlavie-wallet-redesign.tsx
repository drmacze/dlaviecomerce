...
export function DlavieWalletRedesign() {
  ...
  const stats = [
    { label: 'Balance', value: rupiah(wallet?.d_balance || 0), hint: 'D-Balance' },
    { label: 'Points', value: String(wallet?.d_points || 0), hint: `${rewardTotal} earned` },
    { label: 'VIP', value: wallet?.vip_level || 'free', hint: 'Reward tier' },
    { label: 'Orders', value: String(purchases), hint: 'Purchases' }
  ];

  const activityContent = (
    <div className="space-y-3">
      {latest.length ? latest.map((tx) => (
        <div key={tx.id} className="rounded-[1.25rem] border border-white/10 bg-white/[0.045] p-4">
    ...
    <main className="dlavie-system-page min-h-screen px-3 pb-36 pt-4 text-white md:px-6 md:pt-6">
      ...
      <div className="mx-auto max-w-7xl space-y-4">
        <header className="dlv-reveal rounded-[2rem] border border-white/10 bg-white/[0.045] p-4 shadow-[0_24px_80px_rgba(0,0,0,.34)] backdrop-blur-2xl md:p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-white/36">DLAVIE WALLET</p>
              <h1 className="mt-2 text-3xl font-semibold leading-none tracking-[-.06em] md:text-5xl">Wallet center.</h1>
              <p className="mt-3 max-w-2xl text-sm font-medium leading-6 text-white/48">Top up D-Balance, pilih metode pembayaran, dan pantau aktivitas wallet dalam satu ruang yang rapi.</p>
            </div>
            <a href="/products" className="rounded-full bg-white px-4 py-2 text-xs font-semibold text-slate-950">Produk</a>
          </div>
        </header>
        ...
        <section id="wallet-panel" className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="dlv-reveal rounded-[2rem] border border-white/10 bg-white/[0.045] p-4 shadow-[0_28px_90px_rgba(0,0,0,.38)] backdrop-blur-2xl md:p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-white/34">Top up amount</p>
                <h2 className="mt-2 text-4xl font-semibold tracking-[-.07em] text-white">{rupiah(amount)}</h2>
              </div>
              <span className="rounded-full border border-white/10 bg-white px-3 py-1.5 text-[10px] font-semibold text-slate-950">Min 10K</span>
            </div>

            <div className="mt-5 grid grid-cols-3 gap-2">
              {amounts.map((value) => (
                <button key={value} onClick={() => { setSelected(value); setCustom(''); }} className={`rounded-[1.05rem] px-3 py-3 text-sm font-semibold transition ${amount === value && !custom ? 'bg-white text-slate-950' : 'border border-white/10 bg-white/[0.045] text-white/66 hover:bg-white/[0.08]'}`}>
                  {value / 1000}K
                </button>
              ))}
            </div>

            <input inputMode="numeric" value={custom} onChange={(event) => setCustom(event.target.value.replace(/\D/g, ''))} placeholder="Custom amount" className="mt-3 w-full rounded-[1.15rem] border border-white/10 bg-black/20 px-4 py-4 text-sm font-semibold text-white outline-none placeholder:text-white/28 focus:border-white/25" />

            <div className="mt-4 grid grid-cols-2 gap-2">
              <button onClick={() => setMode('auto')} className="rounded-[1.1rem] bg-white px-4 py-3 text-sm font-semibold text-slate-950">Auto Gateway</button>
              <button onClick={() => setMode('manual')} className="rounded-[1.1rem] border border-white/10 bg-white/[0.045] px-4 py-3 text-sm font-semibold text-white/72">Manual</button>
            </div>
          </div>

          <div className="dlv-reveal rounded-[2rem] border border-white/10 bg-white/[0.045] p-4 shadow-[0_28px_90px_rgba(0,0,0,.38)] backdrop-blur-2xl md:p-5">
            <div className="flex gap-2 rounded-[1.25rem] border border-white/10 bg-black/20 p-1.5">
              {(['auto', 'manual', 'activity'] as WalletMode[]).map((item) => (
                <button key={item} onClick={() => setMode(item)} className={`flex-1 rounded-[.95rem] px-3 py-2.5 text-xs font-semibold capitalize transition ${mode === item ? 'bg-white text-slate-950' : 'text-white/48 hover:bg-white/[0.06] hover:text-white'}`}>
                  {item === 'auto' ? 'Auto' : item === 'manual' ? 'Manual' : 'Activity'}
                </button>
              ))}
            </div>
            ...
            <p className="mt-4 rounded-[1.2rem] border border-white/10 bg-white/[0.035] p-3 text-xs font-medium leading-5 text-white/46">{status}</p>
          </div>
        </section>
      </div>
    </main>
