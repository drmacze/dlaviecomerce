...
export async function commerceWebhookRoutes(app: FastifyInstance): Promise<void> {
  app.post('/v1/webhooks/midtrans', async (request, reply) => {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (!event) return { duplicate: true, status: payment.status };
      ...
      if (nextStatus === 'paid' && !payment.terminalProcessedAt) {
        ...
        for (const item of items) {
          ...
          await tx.insert(inventoryMovementsTable).values({
            ...
            reason: 'Payment confirmed by Midtrans.',
            actor: 'midtrans-webhook',
          });
        }
      ...
      if (
        ...
        for (const item of items) {
          ...
          await tx.insert(inventoryMovementsTable).values({
            ...
            type: 'release',
            quantityDelta: item.quantity,
            reason: `Midtrans payment became ${nextStatus}.`,
            actor: 'midtrans-webhook',
          });
        }
    ...
    return reply.status(200).send({ ok: true, duplicate: result.duplicate, status: result.status });
  });
}
