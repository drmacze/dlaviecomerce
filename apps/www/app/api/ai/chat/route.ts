...
const schema = z
  .object({
    ...
    mode: z
      .enum(['fast', 'smart', 'agent', 'research', 'private', 'dlavie', 'general'])
      .default('fast'),
    conversationId: z.string().uuid().optional(),
    metadata: z
      .object({ history_enabled: z.boolean().default(false) })
      .passthrough()
      .default({ history_enabled: false }),
  })
  .strict();
...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      { ok: false, message: 'Invalid chat request.', issues: parsed.error.issues },
      { status: 400 },
    );
  const user = await getDlavieUserFromRequest(request).catch(() => null);
  const preference = user
    ? await import('../../../../src/server/services')
        .then(({ aiPreferencesService }) => aiPreferencesService.get(user.id))
        .catch(() => ({ historyEnabled: false }))
    : { historyEnabled: false };
  const persist = shouldPersistAiHistory({
    authenticated: Boolean(user),
    serverHistoryEnabled: preference.historyEnabled,
    clientHistoryEnabled: parsed.data.metadata.history_enabled,
    mode: parsed.data.mode,
  });
  const providerRequest = chatRequestSchema.parse({
    conversation_id: persist ? parsed.data.conversationId : undefined,
    mode: parsed.data.mode === 'agent' || parsed.data.mode === 'dlavie' ? 'dlavie' : 'general',
    use_rag: parsed.data.mode === 'research',
    stream: false,
