...
const schema = z.object({
  ...
  mode: z.enum(['fast', 'smart', 'agent', 'research', 'private', 'dlavie', 'general']).default('fast'),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, answer: 'DLavie AI sedang memakai mode aman. Jelaskan kebutuhan Anda dalam satu atau dua kalimat dan saya akan membantu semampunya.', source: 'safe-fallback', mode: parsed.data.mode, persisted: false, fallbackUsed: true });
  }
}
