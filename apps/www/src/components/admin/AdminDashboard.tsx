        ...
        <>
          <section className="admin-stat-grid" aria-label="Statistik utama">
            <article>
              ...
              <small>
                {data.products.draft} draft · {data.products.archived} arsip
              </small>
            </article>
            <article>
            ...
            <article data-warning={data.inventory.lowStockVariants > 0 || undefined}>
              ...
              <small>
                {data.inventory.reserved} unit direservasi · {data.inventory.lowStockVariants} SKU
                menipis
              </small>
            </article>
          </section>
          ...
          <section className="admin-panel-grid">
            <article className="admin-panel">
              <div className="admin-panel__heading">
                <h2>Produk</h2>
                <Link href="/admin/products">Kelola</Link>
              </div>
              <dl className="admin-breakdown">
                ...
                <div>
                  <dt>Draft</dt>
                  <dd>{data.products.draft}</dd>
                </div>
                <div>
                  <dt>Aktif</dt>
                  <dd>{data.products.active}</dd>
                </div>
                <div>
                  <dt>Diarsipkan</dt>
                  <dd>{data.products.archived}</dd>
                </div>
              </dl>
            </article>
            <article className="admin-panel">
              <div className="admin-panel__heading">
                <h2>Fulfillment</h2>
                <Link href="/admin/orders">Buka pesanan</Link>
              </div>
              <dl className="admin-breakdown">
                <div>
                  <dt>Dibayar</dt>
                  <dd>{data.orders.paid}</dd>
                </div>
                <div>
                  <dt>Diproses</dt>
                  <dd>{data.orders.processing}</dd>
                </div>
                <div>
                  <dt>Dikirim</dt>
                  <dd>{data.orders.shipped}</dd>
                </div>
                <div>
                  <dt>Selesai</dt>
                  <dd>{data.orders.completed}</dd>
                </div>
              </dl>
            </article>
          </section>
