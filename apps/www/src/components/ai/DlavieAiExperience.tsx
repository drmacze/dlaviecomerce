...
type DlavieAiExperienceProps = {
  accountSession: DlavieAccountSession;
};

export function DlavieAiExperience({ accountSession }: DlavieAiExperienceProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(
        '[data-ai-intro]',
        { y: 22, opacity: 0, filter: 'blur(12px)' },
        { y: 0, opacity: 1, filter: 'blur(0px)', duration: 0.82, stagger: 0.07, ease: 'power3.out' },
      );
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      ...
      { opacity: 0.78, y: 10, filter: 'blur(8px)' },
  ...
  return (
    <main ref={rootRef} className={`dlavie-ai dlavie-ai--${mode}`}>
      ...
      <AiHero mode={mode} modes={modes} content={content} accountSession={accountSession} onModeChange={setMode} />
      ...
      <section className="dlavie-ai__section ai-connectors" aria-labelledby="ai-connectors-title">
        <div className="dlavie-ai__section-heading" data-ai-reveal>
          ...
          <h2 id="ai-connectors-title">One quiet intelligence layer across account, commerce, and operations.</h2>
          <span>DLavie AI is designed as a shared command layer for customer intent and operator action—clear, gated, and easy to understand.</span>
        </div>
      ...
      <section id="modes" className="dlavie-ai__section ai-mode-story" aria-labelledby="ai-mode-title">
        <div ref={modePanelRef} className="ai-mode-story__panel" data-ai-reveal>
          ...
          <h2 id="ai-mode-title">{content.label}: {mode === 'ai' ? 'clean conversation, support, search, and guidance.' : 'structured workflows, tool plans, and guarded automation.'}</h2>
          <p>{content.subcopy}</p>
      ...
      <section className="dlavie-ai__section ai-use-cases" aria-labelledby="ai-use-cases-title">
        <div className="dlavie-ai__section-heading" data-ai-reveal>
          ...
          <h2 id="ai-use-cases-title">Built for daily pressure without visual noise.</h2>
          <span>This is the operating front door for DLavie support, commerce, and product work—not a generic chatbot landing page.</span>
        </div>
      ...
      <section className="dlavie-ai__section ai-thinking-demo" aria-labelledby="ai-thinking-title">
        <div className="ai-thinking-demo__copy" data-ai-reveal>
          ...
          <h2 id="ai-thinking-title">Readable AI state before the answer appears.</h2>
      ...
      <AiPricing plans={accessPlans} isAuthenticated={isAuthenticated} />
      ...
      <section className="dlavie-ai__section ai-insights" aria-labelledby="ai-insights-title">
        <div className="dlavie-ai__section-heading" data-ai-reveal>
          ...
          <h2 id="ai-insights-title">Premium AI product depth, designed for trust.</h2>
        </div>
      ...
      <AiFinalCta accountSession={accountSession} />
    </main>
  );
}
