...
function migrationNumber(filename) {
  ...
  const value = Number.parseInt(separator === -1 ? filename : filename.slice(0, separator), 10);
  if (!Number.isSafeInteger(value) || value < 0) {
    throw new Error(`Migration ${filename} does not start with a valid numeric sequence.`);
  }
  return value;
}
...
try {
  ...
  await client
    .query("select pg_advisory_unlock(hashtext($1))", ["dlavie-commerce-migrations"])
    .catch(() => undefined);
  client.release();
  await pool.end();
}
