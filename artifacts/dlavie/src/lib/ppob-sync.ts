...
function defaultMargin(product: DigiflazzPriceProduct) {
  const byCategory: Record<string, number> = {
    pulsa: toInt(import.meta.env.PPOB_MARGIN_PULSA || 1000),
    data: toInt(import.meta.env.PPOB_MARGIN_DATA || 1500),
    games: toInt(import.meta.env.PPOB_MARGIN_GAME || 1500),
    game: toInt(import.meta.env.PPOB_MARGIN_GAME || 1500),
    pln: toInt(import.meta.env.PPOB_MARGIN_PLN || 1500),
    voucher: toInt(import.meta.env.PPOB_MARGIN_VOUCHER || 1500)
  };

  const category = clean(product.category).toLowerCase();
  return byCategory[category] ?? toInt(import.meta.env.PPOB_DEFAULT_MARGIN || 1500);
}
...
export function hasDigiflazzEnv() {
  return Boolean(import.meta.env.DIGIFLAZZ_USERNAME && import.meta.env.DIGIFLAZZ_API_KEY);
}
