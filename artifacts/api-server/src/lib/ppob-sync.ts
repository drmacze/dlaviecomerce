...
async function fetchDigiflazzPriceList() {
  const { fetchDigiflazzPriceList: fetch } = await import('./digiflazz.js');
  return fetch('prepaid');
}
...
export async function syncDigiflazzPrepaidProducts() {
  ...
  const rows = products
    .filter((item: DigiflazzPriceProduct) => clean(item.buyer_sku_code) && clean(item.product_name))
    .map((item: DigiflazzPriceProduct) => {
