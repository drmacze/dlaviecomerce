...
async function recordEvent(supabase: SupabaseService, input: { orderId?: string | null; userId?: string | null; eventType: string; message?: string; metadata?: Record<string, unknown> }) {
  try {
    await supabase.from('panel_order_events').insert({ order_id: input.orderId || null, user_id: input.userId || null, event_type: input.eventType, message: input.message || null, metadata: input.metadata || {} });
  } catch {}
}

router.get('/panel/products', async (_req, res) => {
  const supabase = createSupabaseServiceClient();
  const result = await supabase.from('panel_products').select('*').eq('is_active', true).order('sort_order', { ascending: true }).order('price', { ascending: true });
  if (result.error) return res.status(500).json({ error: result.error.message });
  return res.status(200).json({ products: result.data || [] });
});

router.get('/panel/orders', async (req, res) => {
  ...
  const supabase = createSupabaseServiceClient();
  const result = await supabase.from('panel_orders').select('*, panel_products(name, slug, badge)').eq('user_id', user.id).order('created_at', { ascending: false }).limit(30);
  if (result.error) return res.status(500).json({ error: result.error.message });
  return res.status(200).json({ orders: result.data || [] });
});

router.post('/panel/orders', async (req, res) => {
  const user = await verifySupabaseUser(bearerToken(req.headers.authorization));
  ...
  const explicitKey = cleanIdempotencyKey(headerKey || req.body?.idempotency_key);
  const idempotencyKey = explicitKey ? `client:${explicitKey}` : `auto:${productId}:${requestedUsername}:${Math.floor(Date.now() / 30000)}`;

  const existingOrder = await supabase.from('panel_orders').select('*').eq('user_id', user.id).eq('idempotency_key', idempotencyKey).maybeSingle();
  ...
  const productResult = await supabase.from('panel_products').select('*').eq('id', productId).eq('is_active', true).single();
  ...
  try {
    ...
    return res.status(400).json({ error: error instanceof Error ? error.message : 'D-Balance gagal dipotong.' });
  }

  const walletResult = await supabase.from('wallet_transactions').insert({ user_id: user.id, type: 'purchase', amount, status: 'success', provider: 'panel-manual', reference: publicOrderId, metadata: { source: 'panel-order', public_order_id: publicOrderId, product_id: product.id, product_slug: product.slug, product_name: product.name, requested_username: requestedUsername, server_name: serverName, deducted_balance: amount, balance_after: nextBalance, idempotency_key: idempotencyKey, created_at: now } }).select('*').single();
  ...
  const orderResult = await supabase.from('panel_orders').insert({ user_id: user.id, product_id: product.id, public_order_id: publicOrderId, service_type: 'pterodactyl_panel', status: 'pending_fulfillment', product_snapshot: product, requested_username: requestedUsername, server_name: serverName, notes: notes || null, amount, wallet_transaction_id: walletResult.data.id, idempotency_key: idempotencyKey, raw_response: { mode: 'manual_fulfillment', created_at: now } }).select('*').single();
  ...
  await recordEvent(supabase, { orderId: orderResult.data.id, userId: user.id, eventType: 'order_created', message: 'Order panel dibuat dan menunggu proses admin.', metadata: { public_order_id: publicOrderId, product_slug: product.slug, amount, balance_after: nextBalance } });
  await recordEvent(supabase, { orderId: orderResult.data.id, userId: user.id, eventType: 'balance_debited', message: 'D-Balance dipotong untuk pembelian panel.', metadata: { amount, balance_after: nextBalance, wallet_transaction_id: walletResult.data.id } });

  return res.status(201).json({ order: orderResult.data, wallet: { d_balance: nextBalance }, message: 'Order panel berhasil dibuat. Admin akan memproses dan mengirim detail panel ke dashboard kamu.' });
});
