...
export async function settlePpobOrder(refId: string, payload: DigiflazzTransactionData, source: string) {
  ...
  if (!order.data) return { ok: false, status: 'missing', order: null, refund: null, reason: 'ppob-order-not-found' };
  ...
  if (normalized === 'failed') {
    ...
    return { ok: true, status: nextStatus, order: updated.data, refund, reason: null };
  }

  return { ok: true, status: nextStatus, order: updated.data, refund: null, reason: null };
}
